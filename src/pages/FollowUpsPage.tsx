...
export function FollowUpsPage() {
  ...
  const { data: followUps, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('follow_ups')
        .select('*, patients(full_name, uhid, phone), visits(clinic_module)')
  ...
  return (
    <div>
      ...
      <div className="seg" style={{ maxWidth: 560, marginBottom: 'var(--space-4)' }}>
      ...
      {isLoading ? <p className="text-muted">Loading…</p> : (
        <table className="table">
          <thead><tr><th>Due date</th><th>Patient</th><th>UHID</th><th>Phone</th><th>Clinic</th><th>Reason</th><th>Status</th><th /></tr></thead>
          <tbody>
            {filtered.map((f: any) => {
            ...
            {filtered.length === 0 && <tr><td colSpan={8} className="text-muted">Nothing here.</td></tr>}
          </tbody>
        </table>
      )}
    </div>
