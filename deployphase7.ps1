# Netra HIMS - Diagrams/pictures for consultation + printable outputs +
# editable clinical records + drug-master wiring + BOM cleanup
#
# - Diagram tool: upload a real photo to annotate on top of, not just a template
# - New: printable Consultation Report, Prescription slip, Discharge summary,
#   MLC police intimation, Optical order slip
# - Every clinical stage record (~30 screens) can now be edited, not just added
# - Retina/injection drug names now pull from the real pharmacy formulary
# - Stray UTF-8 BOM stripped from every affected file
#
...
New-Item -ItemType Directory -Force -Path 'src\pages' | Out-Null
Set-Content -Path 'src\pages\MrdMlcRegisterPage.tsx' -Encoding UTF8 -NoNewline -Value @'
import { useState } from 'react';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { supabase } from '../lib/supabaseClient';
import { useAuth } from '../lib/AuthContext';
...
'@
Write-Host '  wrote src\pages\MrdMlcRegisterPage.tsx'
...
New-Item -ItemType Directory -Force -Path 'src\lib' | Out-Null
Set-Content -Path 'src\lib\printDischargeSummary.ts' -Encoding UTF8 -NoNewline -Value @'
import { supabase } from './supabaseClient';
...
'@
Write-Host '  wrote src\lib\printDischargeSummary.ts'

New-Item -ItemType Directory -Force -Path 'src\lib' | Out-Null
Set-Content -Path 'src\lib\printMlcIntimation.ts' -Encoding UTF8 -NoNewline -Value @'
import { supabase } from './supabaseClient';
...
'@
Write-Host '  wrote src\lib\printMlcIntimation.ts'

New-Item -ItemType Directory -Force -Path 'src\lib' | Out-Null
Set-Content -Path 'src\lib\printOpticalOrderSlip.ts' -Encoding UTF8 -NoNewline -Value @'
import { supabase } from './supabaseClient';
...
'@
Write-Host '  wrote src\lib\printOpticalOrderSlip.ts'

New-Item -ItemType Directory -Force -Path 'src\lib' | Out-Null
Set-Content -Path 'src\lib\printPrescriptionSlip.ts' -Encoding UTF8 -NoNewline -Value @'
import { supabase } from './supabaseClient';
...
'@
Write-Host '  wrote src\lib\printPrescriptionSlip.ts'

Write-Host 'All 63 files written.' -ForegroundColor Green
git add -A
git status
Write-Host "Now run: git commit -m 'Add photo-based diagrams, printable clinical reports, editable records, drug-master wiring' ; git push origin claude/determined-ride-o63al9" -ForegroundColor Yellow